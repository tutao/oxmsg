...
export function fileTimeToDate(lower: number, upper: number): Date {
	// $FlowIgnore[bigint-unsupported]
	const fileTime = BigInt.asUintN(64, BigInt(lower) + (BigInt(upper) << 32n))
	// $FlowIgnore[bigint-unsupported]
	const filetimeMillis = fileTime / 10000n
	// $FlowIgnore[bigint-unsupported]
	const unixtime = filetimeMillis - 11644473600n
	return new Date(Number(unixtime))
}

export function dateToFileTime(date: Date): { lower: number, upper: number } {
	const unixtime = BigInt(date.getTime())
	// $FlowIgnore[bigint-unsupported]
	const fileTime = (unixtime + 11644473600n) * 10000n
	const lower = Number(fileTime) & Number.MAX_SAFE_INTEGER
	// $FlowIgnore[bigint-unsupported]
