...
export function to(localeId: string): number {
    if (typeof localeId !== 'string') throw new TypeError('Expected a string')

    const lcidCode = inverted[localeId]
    if (lcidCode) return Number(inverted[localeId])
}
