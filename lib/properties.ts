...
export class Properties extends Array<Property> {
    ...
    _expectPropertyType(expected: PropertyType, actual: PropertyType) {
        if (actual !== expected) {
            throw new Error(`Invalid PropertyType "${PropertyType[actual]}". Expected "${PropertyType[expected]}"`)
        }
    }

    addDateProperty(tag: PropertyTagsEnum, value: Date, flags: number = DEFAULT_FLAGS) {
        this._expectPropertyType(PropertyType.PT_SYSTIME, tag.type)

        this._addProperty(tag, dateToFileTime(value), flags)
    }

    addBinaryProperty(tag: PropertyTagsEnum, data: Uint8Array, flags: number = DEFAULT_FLAGS) {
     ...
     * @param flags
     */
    addProperty(tag: PropertyTagsEnum, value: any, flags: number = DEFAULT_FLAGS): void {
