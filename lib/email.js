...
import {MessageHeader} from "./mime/header/message_header"
...
export class Email extends Message {
	...
	_writeToStorage(): void {
		...
		if (this.transportMessageHeaders != null) {
			this._topLevelProperties.addProperty(PropertyTags.PR_TRANSPORT_MESSAGE_HEADERS_W, this.transportMessageHeaders.toString())

			if (!isNullOrWhiteSpace(this.transportMessageHeaders.messageId))
				this._topLevelProperties.addProperty(PropertyTags.PR_INTERNET_MESSAGE_ID_W, this.transportMessageHeaders.messageId)

			const refCount = this.transportMessageHeaders.references.length
			if (refCount > 0)
				this._topLevelProperties.addProperty(PropertyTags.PR_INTERNET_REFERENCES_W, this.transportMessageHeaders.references[refCount - 1])

			const replCount = this.transportMessageHeaders.inReplyTo.length
			if (replCount > 0)
				this._topLevelProperties.addProperty(PropertyTags.PR_IN_REPLY_TO_ID_W, this.transportMessageHeaders.inReplyTo[replCount - 1])
		}
		...
		if (this.sentOn == null) this.sentOn = new Date()

		// TODO: receivedOn.HasValue?
		if (this.receivedOn != null)
			this._topLevelProperties.addProperty(PropertyTags.PR_MESSAGE_DELIVERY_TIME, dateToFileTime(this.receivedOn))

		// was SentOn.Value.ToUniversalTime()
		this._topLevelProperties.addProperty(PropertyTags.PR_CLIENT_SUBMIT_TIME, dateToFileTime(this.sentOn))
