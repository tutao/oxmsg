...
export class Email extends Message {
	...
	inReplyToId: string
	// TODO we want to be able to provide the headers raw, as a string
	...
	_writeToStorage(): void {
		...
		if (this.transportMessageHeaders != null) {
			...
			// TODO transportMessageHeaders.toString() will output "[object Object]", this isn't what we want
			...
			if (refCount > 0) {
				this._topLevelProperties.addProperty(PropertyTags.PR_INTERNET_REFERENCES_W, this.transportMessageHeaders.references[refCount
				- 1])
			}

			const replCount = this.transportMessageHeaders.inReplyTo.length
			if (replCount > 0) {
				this._topLevelProperties.addProperty(PropertyTags.PR_IN_REPLY_TO_ID_W, this.transportMessageHeaders.inReplyTo[replCount
				- 1])
			}
		}
