...
o.spec("rfc2047", function () {
    o.spec("#encode() and #decode()", function () {
        ...
        o("should handle a string that does not need to be encoded", function () {
            expectRoundtrip("Andreas Lind <andreas@one.com>", "Andreas Lind <andreas@one.com>")
        })
        o("should handle a multi-word string where the middle word has to be encoded", function () {
            expectRoundtrip("Andreas Lindø <andreas@one.com>", "Andreas =?utf-8?Q?Lind=C3=B8?= <andreas@one.com>")
        })
        o("should use an UTF-8 encoded word when a character is not in iso-8859-1", function () {
            expectRoundtrip("Mr. Smiley face aka ☺ <smiley@face.dk>", "Mr. Smiley face aka =?utf-8?Q?=E2=98=BA?= <smiley@face.dk>")
        })
        o("should handle two neighbouring words that have to be encoded", function () {
            expectRoundtrip("¡Hola, señor!", "=?utf-8?Q?=C2=A1Hola=2C?= =?utf-8?Q?_se=C3=B1or!?=")
            expectRoundtrip("På lördag", "=?utf-8?Q?P=C3=A5?= =?utf-8?Q?_l=C3=B6rdag?=")
        })
        o("should not rely on the space between neighbouring encoded words to be preserved", function () {
            expectRoundtrip("☺ ☺", "=?utf-8?Q?=E2=98=BA?= =?utf-8?Q?_=E2=98=BA?=")
        })
        o("should handle some dreamed up edge cases", function () {
            expectRoundtrip("lördag", "=?utf-8?Q?l=C3=B6rdag?=")
        })
        o("should handle a multi-word string where the middle word has to be left unencoded", function () {
            expectRoundtrip("Så er fødselen i gang", "=?utf-8?Q?S=C3=A5?= er =?utf-8?Q?f=C3=B8dselen?= i gang")
        })
        o("should place leading quotes correctly", function () {
            expectRoundtrip('"ÅÄÖ" <sss@example.com>', '"=?utf-8?Q?=C3=85=C3=84=C3=96?=" <sss@example.com>')
        })
        o("should place trailing quotes correctly", function () {
            expectRoundtrip('"TEST ÅÄÖ" <sss@example.com>', '"TEST =?utf-8?Q?=C3=85=C3=84=C3=96?=" <sss@example.com>')
        })
        // Regression test for #2:
        o("should handle an emoji test case", function () {
            expectRoundtrip('{"tags":"","fullName":"😬"}', '=?utf-8?Q?{=22tags=22=3A?=""=?utf-8?Q?=2C=22fullNa?= =?utf-8?Q?me=22=3A=22=F0=9F=98=AC=22?=}')
        })
        o("should handle the replacement character", function () {
            expectRoundtrip("test_�.docx", "=?utf-8?Q?test=5F=EF=BF=BD=2Ed?=ocx")
        })
    })
    ...
    o.spec("#decode()", function () {
        ...
        o("should handle the test cases listed in RFC 2047", function () {
            expectDecoding("=?ISO-8859-1?Q?Olle_J=E4rnefors?= <ojarnef@admin.kth.se>", "Olle Järnefors <ojarnef@admin.kth.se>")
            expectDecoding("=?ISO-8859-1?Q?Patrik_F=E4ltstr=F6m?= <paf@nada.kth.se>", "Patrik Fältström <paf@nada.kth.se>")
            expectDecoding(
        ...
        o("should handle subject found in mail with X-Mailer: MailChimp Mailer", function () {
            ...
            expectDecoding("=?iso-8859-1?Q?Spar 20 %...?=", "Spar 20 %...")
        })
        ...
        o("should decode a UTF-8 smiley (illegally) split up into 2 encoded words", function () {
            expectDecoding("=?utf-8?Q?=E2=98?= =?utf-8?Q?=BA?=", "☺")
        })
        o("should decode a UTF-8 smiley (illegally) split up into 3 encoded words", function () {
            expectDecoding("=?utf-8?Q?=E2?= =?utf-8?Q?=98?= =?utf-8?Q?=BA?=", "☺")
        })
        o("should give up decoding a UTF-8 smiley (illegally) split up into 3 encoded words if there is regular text between the encoded words", function () {
            expectDecoding("=?utf-8?Q?=E2?= =?utf-8?Q?=98?=a=?utf-8?Q?=BA?==?utf-8?Q?=BA?=a", "=?utf-8?Q?=E2?==?utf-8?Q?=98?=a=?utf-8?Q?=BA?==?utf-8?Q?=BA?=a")
        })
        o("should decode an encoded word following a undecodable sequence of encoded words", function () {
            expectDecoding("=?utf-8?Q?=E2?= =?utf-8?Q?=98?= =?iso-8859-1?Q?=A1?=Hola, se=?iso-8859-1?Q?=F1?=or!", "=?utf-8?Q?=E2?==?utf-8?Q?=98?=¡Hola, señor!")
        })
        o("should handle test cases from the MIME tools package", function () {
            ...
            expectDecoding("=?ISO-8859-1?Q?Keld_J=F8rn_Simonsen?= <keld@dkuug.dk>", "Keld Jørn Simonsen <keld@dkuug.dk>")
            expectDecoding("=?US-ASCII?Q?Keith_Moore?= <moore@cs.utk.edu>", "Keith Moore <moore@cs.utk.edu>")
            expectDecoding("=?ISO-8859-1?Q?Andr=E9_?= Pirard <PIRARD@vm1.ulg.ac.be>", "André  Pirard <PIRARD@vm1.ulg.ac.be>")
        ...
        o("should handle a file name found in a Korean mail", function () {
            expectDecoding("=?ks_c_5601-1987?B?MTMwMTE3X8HWwvfA5V+1tcDlX7jetLq+8y5wZGY=?=", "130117_주차장_도장_메뉴얼.pdf")
        })
