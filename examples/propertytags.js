...
function fromIdAndType(id, type) {
	const withSameId = Object.keys(tags).filter(k => tags[k].id.toString(16).padStart(4, '0').toUpperCase() === id)
	return withSameId.find(k => tags[k].type.toString(16).padStart(4, '0').toUpperCase() === type)
}

export function getTag(s) {
	const parts = s.split(' ')
	const id = parts[3] + parts[2]
	const type = parts[1] + parts[0]
	return fromIdAndType(id, type)
}


export function getTagFromStreamName(name) {
	const id = name.slice(-8, -4)
