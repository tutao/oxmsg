...
export class HeaderFieldParser {
     ...
     */
    static parseContentTransferEncoding(value: string): ContentTransferEncodingEnum {
     ...
     */
    static parseImportance(value: string): MailPriorityEnum {
        if (value == null) throw new Error("value must not be null!")
            ...
            switch (key) {
                case "":
                    ...
                    const v = value.toUpperCase()
                        .replace(/^[\x00]/, "")
                        .replace(/[\x00]$/, "")
                    contentType.mediaType = v === "TEXT" || v === "TEXT/"
                        ? "text/plain"
                        : value
                    break
                case "BOUNDARY":
    ...
    static parseContentDisposition(value: string): ContentDisposition {
        ...
        parameters.forEach(kvp => {
            ...
            switch (key) {
                ...
                default:
                    if (!key.startsWith("X-")) throw new Error(
