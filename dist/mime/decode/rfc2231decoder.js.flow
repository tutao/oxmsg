...
export class Rfc2231Decoder {
    ...
    static decode(input: string): KeyValueList {
        ...
        const parts = splitAtUnquoted(input.trim(), ';')
        ...
        parts.forEach(p => {
            ...
            if (eqIdx === -1) collection.push({key: "", value: p})
            else collection.push({key: p.slice(0, eqIdx), value: p.slice(eqIdx)})
        })
     ...
     */
    static decodeSingleValue(input: string, encoding: ?string): string {
        if (input == null) throw new Error("input must not be null!")
